...
class PrismaConcatenator:
    ...
    def read_Prisma_segy(
        ...
        for index, row in self.file_data[self.index:].iterrows():
            ...
            if self.previous_dir is None or self.previous_dir != self.current_dir:
                ...
                if self.previous_gauge_m is not None and self.previous_gauge_m != self.gauge_m:
                ...
                with open(os.path.join(self.parent_input_dir, self.current_dir, row["files"]), "rb") as f:
                    f.seek(3714)
            ...
            end_time = start_time + round((segy_trc_size - 1) / sampling_rate, 1)
            ...
            elif self.index < len(self.file_data) -1 and round(self.time_diff[self.index], 0) > unit_size:
    ...
    def save_data(self, start_time):
        ...
        """
        data_start_date = datetime.datetime.fromtimestamp(start_time).strftime("%Y%m%d")
        if not os.path.exists(os.path.join(self.parent_output_dir, data_start_date)):
            os.makedirs(os.path.join(self.parent_output_dir, data_start_date))
        else: 
            print(os.path.join(self.parent_output_dir, data_start_date))
        h5_concat = os.path.join(
            self.parent_output_dir,
            data_start_date,
            f"{data_start_date}_{start_time}.hdf5",
        )
    ...
    def run(self):
        ...
        self.time_diff = np.diff(self.file_data["timestamps"])
        self.process_dir()
